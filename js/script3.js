...
d3.select("#myCheckbox4").on("change",check);
d3.select("#myCheckbox5").on("change",check);
d3.select("#myCheckbox6").on("change",check);
var checkline1 = 0
var checkline2 = 0
var checkline3 = 1
function check(){
...
function update3(){
  var dateUpdate = document.getElementById("date3").value
  d3.select("#visual3").selectAll("*").remove();
...
//Read the data
  d3.csv("https://raw.githubusercontent.com/hoaluan97/dataviz-electricite-conso/master/Sum_30Jours_FilterYearMonth.csv",

    // When reading the csv, I must format variables:
    function(d){
          if(d.Year==dateUpdate.split("/")[2] && d.Month==dateUpdate.split("/")[1]){
            return {date : d3.timeParse("%d/%m/%Y")(d.Day_of_month + "/" + d.Month +
                                                   "/" + d.Year),
                    Global_active_power : d.Global_active_power_Sum,
                    Sub_metering_1 : d.Sub_metering_1_Sum,
                    Sub_metering_2 : d.Sub_metering_2_Sum,
                    Sub_metering_3 : d.Sub_metering_3_Sum,
